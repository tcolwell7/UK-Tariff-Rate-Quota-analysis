...
#'                 - Removal of function - unnecessary complication to script. Full audit would require data cleaning at start of script before applying function across TRQ/sector level figures. 
...
#'                 - Input file created to align quota units between UK and EU data. 
#'                 - Quota unit final added and HL converted to L. Quota unit added to group by aggregation within functions. 
...
#' - v7  - 01/03/22 - outputs re-run with Thailand removed. This was incorrectly in region list. UK country input file updated. QA sections removed as they are now redundant. 
#'       - 02/03/22 - Nicaragua grouping updated in country input and outputs re-run. 
#' - v8  - 14/03/22 - Issue with quota balancing being larger than quota volumes - essentially creating "negative" usage. 
#'                   Quotas can carry over un-used quantities; this is the casue for larger balances than volumes. 
#'                   Solution: where balance > quota volume set the balance to volume figure. This is correct way forward and won't affect the calculations as all quotas have 0 usage where this affects.(line updated in cleaning section for UK data)
#' - v9  - 17/03/22 - Update in UK TRQ data. Removal of specific quotas which quota figures have chnaged from February to March which is an active error currently being investigated.     
#' - v10 - 16/04/22 - Update to EU input files for small correction to country names. Upade of underlying UK TRQ data inputs following data updates. New version created to track outputs.       
#'      
...
uk_aggFunction <- function(df, .grouping = NULL){
  ...
  # extract dataframe name to check if exists.
  ...
  for(g in c("grouping","region","quota_origin")){
    for(c in unique(uk_trq$region)){
      ...
      # !!as.symbol extraxts string to create variables within loop
      if(!is.null(.grouping)){
...
uk_trq <- read_excel("..\\data\\uk_trq_data.xlsx") %>% 
  ...
  mutate(
    remaining_balance = 
      ifelse(remaining_balance > quota_volume, quota_volume, remaining_balance) # if balance > volume set to overall volume so no skewed end allocation fill rate figures. 
...
# manual update to remove TRQs with known data issues:

uk_trq <-
  uk_trq %>%
  filter(
    quota_number %notin% 
      c("058025","058040", "058866", "058868", "058878","058971","058974","058975")
)
...
saveWorkbook(wb3, file = "..\\outputs\\trq_data_output10.xlsx", overwrite = T)
...
write.xlsx(uk_trqs_filled, "..\\outputs\\uk_trqs_filled10.xlsx", overwrite = T)
...
saveWorkbook(wb2, file = "..\\outputs\\trq_sector_data_output10.xlsx", overwrite = T)
...
write.xlsx(uk_trqs_filled_sector, "..\\outputs\\uk_trqs_filled_sector10.xlsx", overwrite = T)

## 6. Save Rdata (all dfs) ----------------------------------------------------

save(trqs_country, trqs_grouping, trqs_region, uk_trqs_filled, 
     sectors_country, sectors_grouping, sectors_region, uk_trqs_filled_sector,uk_trq_sector2,
     file = "..\\data\\trq_data_all10.RData")
